...
namespace InsureFlowAPI.Controllers
{
    ...
    public class PolicyController : ControllerBase
    {
        ...
        [Authorize(Roles = "Customer")]
        [HttpPost("purchase")]
        public async Task<IActionResult> PurchasePolicy(PurchasePolicyRequestDto dto)
        {
            int customerUserId = int.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
            
            var policy = await _policyService.PurchasePolicyAsync(customerUserId, dto);
        ...
        [Authorize(Roles = "Admin,InternalStaff")]
        [HttpPost("issue")]
        public async Task<IActionResult> IssuePolicy(IssuePolicyRequestDto dto)
        {
            var policy = await _policyService.IssuePolicyByInternalStaffAsync(dto);
