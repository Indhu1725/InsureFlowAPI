...
namespace InsureFlowAPI.Repositories
{
    public class PolicyPlanRepository : IPolicyPlanRepository
    {
        ...
        public PolicyPlanRepository(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<PagedResponse<PolicyPlan>> GetAllAsync(PolicyPlanQueryDto query)
        {
            ...
            {
                plans = plans.Where(p => p.ProductId == query.ProductId.Value);
            }
            ...
            plans = query.SortBy.ToLower() switch
            {
                ...
                "durationyears" => query.Descending
                    ? plans.OrderByDescending(p => p.DurationYears)
                    : plans.OrderBy(p => p.DurationYears),
