...
namespace InsureFlowAPI.Services.Implementations
{
    public class PremiumPaymentService : IPremiumPaymentService
    {
        ...
        public PremiumPaymentService(IPremiumPaymentRepository premiumPaymentRepository,
        IPolicyRepository policyRepository,IMapper mapper,ILogger<PremiumPaymentService> logger)
        {
        ...
        public async Task<PremiumPaymentResponseDto> MakePaymentAsync(PremiumPaymentRequestDto requestDto,int userId,string role)
        {
            ...
            if (role == "Customer")
            {
                ...
                {
                    _logger.LogWarning("Unauthorized payment attempt. UserId: {UserId}, PolicyId: {PolicyId}",userId,policy.PolicyId);
            ...
            _logger.LogInformation("Recording premium payment. PolicyId: {PolicyId}, CustomerId: {CustomerId}, Amount: {Amount}",
            payment.PolicyId,payment.CustomerId,payment.Amount);
